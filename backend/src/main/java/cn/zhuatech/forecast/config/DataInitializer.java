...
@Bean CommandLineRunner seed(OperatingUnitRepository units,WorkRecordRepository records,ResourceRegisterRepository resources,ReviewRecordRepository reviews,UserRepository users,PasswordEncoder encoder){return args->{if(units.count()>0)return;
 ...
 reviews.saveAll(List.of(new ReviewRecord("FVA-260801-032",a,"预测增值",200,8,ReviewRecord.Result.PENDING,"程望"),new ReviewRecord("FVA-260801-011",b,"促销准确率",120,2,ReviewRecord.Result.PASSED,"叶澜"),new ReviewRecord("FVA-260728-018",c,"决策风险",80,4,ReviewRecord.Result.FAILED,"程望")));
 String demo=encoder.encode("Demo@2026"); users.saveAll(List.of(new UserAccount("operator",demo,"叶澜",UserAccount.Role.DOMAIN_USER,"FCST-EAST"),new UserAccount("planner",demo,"程望",UserAccount.Role.DOMAIN_OPERATOR,null),new UserAccount("quality",demo,"许研",UserAccount.Role.QUALITY,null),new UserAccount("admin",encoder.encode("ZhuaTech@2026"),"系统管理员",UserAccount.Role.ADMIN,null)));};}}
